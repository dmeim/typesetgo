...
function ColorPickerPanel({ hex, onChange }: { hex: string; onChange: (hex: string) => void }) {
    ...
    <div className="flex flex-col gap-4 select-none items-center w-full">
      <div className="relative flex items-center justify-center">
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
            <SatValBox hsv={hsv} onChange={(s, v) => updateColor({ ...hsv, s, v })} />
        </div>
      </div>
...
function HueWheel({ hsv, onChange }: { hsv: { h: number }; onChange: (h: number) => void }) {
...
function SatValBox({ hsv, onChange }: { hsv: { h: number; s: number; v: number }; onChange: (s: number, v: number) => void }) {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const [isDragging, setIsDragging] = useState(false);
