...
import { SettingsState, Mode, Difficulty, QuoteLength, DEFAULT_THEME } from "@/lib/typing-constants";
    ...
    >
      <div className="flex items-center gap-3 overflow-hidden">
        ...
        <div className="flex flex-col overflow-hidden">
          <span className="font-medium text-gray-200 truncate">{item.metadata.title || "Untitled Step"}</span>
...
export default function PlanBuilderModal({ 
  ...
  const handleUpdateItem = (id: string, updates: Partial<PlanItem> | ((prev: PlanItem) => Partial<PlanItem>)) => {
          ...
          {selectedItem ? (
            <div className="space-y-8 max-w-2xl mx-auto animate-fade-in">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-bold text-gray-200">Step Configuration</h3>
                <span className="text-xs text-gray-500 font-mono">{selectedItem.id.slice(0, 8)}</span>
              </div>

              {/* Metadata Section */}
              <div className="space-y-4 p-4 rounded-lg border border-gray-700 bg-gray-800/30">
                <h4 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Display Metadata</h4>
                <div className="grid gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleUpdateItem(selectedItem.id, { metadata: { ...selectedItem.metadata, title: e.target.value } })}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleUpdateItem(selectedItem.id, { metadata: { ...selectedItem.metadata, subtitle: e.target.value } })}
                      className="w-full bg-gray-900 border border-gray-700 rounded p-2 text-gray-200 focus:border-sky-500 outline-none"
              ...
              <div className="space-y-4 p-4 rounded-lg border border-gray-700 bg-gray-800/30">
                 ...
                 {selectedItem.mode === "words" && (
                   <div className="space-y-4">
                     <div>
                       <label className="block text-xs text-gray-500 mb-2">Word Count</label>
                       <div className="flex gap-2">
                         {[10, 25, 50, 100].map(w => (
                           <button
                             key={w}
                             onClick={() => handleUpdateItem(selectedItem.id, { settings: { ...selectedItem.settings, wordTarget: w } })}
                             className={`px-3 py-1 rounded text-sm ${selectedItem.settings.wordTarget === w ? "bg-sky-600 text-white" : "bg-gray-700 text-gray-300"}`}
                           >
                             {w}
                           </button>
                         ))}
                          <input
                            ...
                            onChange={(e) => handleUpdateItem(selectedItem.id, { settings: { ...selectedItem.settings, wordTarget: parseInt(e.target.value) || 0 } })}
                            className="w-20 bg-gray-900 border border-gray-700 rounded px-2 text-center text-gray-200"
                         />
                       </div>
                     </div>
                     ...
                     <div>
                       ...
                       <div className="flex gap-2">
                         {(["all", "short", "medium", "long", "xl"] as QuoteLength[]).map(l => (
                           <button
                             key={l}
                             onClick={() => handleUpdateItem(selectedItem.id, { settings: { ...selectedItem.settings, quoteLength: l } })}
                             className={`px-3 py-1 rounded text-sm capitalize ${selectedItem.settings.quoteLength === l ? "bg-sky-600 text-white" : "bg-gray-700 text-gray-300"}`}
                           >
                             {l}
                           </button>
                         ))}
                 ...
                 {selectedItem.mode !== "quote" && selectedItem.mode !== "zen" && (
                   <div className="flex gap-4 pt-2">
                     <button
                       ...
                       className={`flex items-center gap-2 px-3 py-1.5 rounded transition-colors ${selectedItem.settings.punctuation ? "bg-sky-900/50 text-sky-400 border border-sky-500/50" : "bg-gray-700 text-gray-400"}`}
                     >
                       <span className="font-bold">@</span> Punctuation
                     </button>
                     <button
                       onClick={() => handleUpdateItem(selectedItem.id, { settings: { ...selectedItem.settings, numbers: !selectedItem.settings.numbers } })}
                       className={`flex items-center gap-2 px-3 py-1.5 rounded transition-colors ${selectedItem.settings.numbers ? "bg-sky-900/50 text-sky-400 border border-sky-500/50" : "bg-gray-700 text-gray-400"}`}
                     >
                       ...
                       <div className="flex flex-wrap gap-2">
                         {(["beginner", "easy", "medium", "hard", "extreme"] as Difficulty[]).map(d => (
                           <button
                             ...
                             className={`px-3 py-1 rounded text-sm capitalize ${selectedItem.settings.difficulty === d ? "bg-sky-600 text-white" : "bg-gray-700 text-gray-300"}`}
                           >
                             {d}
                           </button>
                         ))}
              ...
              {isConnectMode && (
                <div className="space-y-4 p-4 rounded-lg border border-indigo-500/30 bg-indigo-900/10">
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-gray-200 font-medium">Wait for All</div>
                        <div className="text-xs text-gray-500">Require everyone to finish before moving on</div>
                      </div>
                      <button
                        onClick={() => handleUpdateItem(selectedItem.id, { syncSettings: { ...selectedItem.syncSettings, waitForAll: !selectedItem.syncSettings.waitForAll } })}
                        className={`w-12 h-6 rounded-full transition-colors relative ${selectedItem.syncSettings.waitForAll ? "bg-indigo-500" : "bg-gray-700"}`}
                      >
                        <span className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white transition-transform ${selectedItem.syncSettings.waitForAll ? "translate-x-6" : "translate-x-0"}`} />
                      </button>
                    </div>
                    ...
                    {selectedItem.syncSettings.waitForAll && (
                      <div className="flex items-center justify-between pl-4 border-l-2 border-indigo-500/30">
                        <div>
                          <div className="text-gray-200 font-medium">Allow Zen Waiting</div>
                          <div className="text-xs text-gray-500">Early finishers enter a Zen mode room</div>
                        </div>
                        <button
                          onClick={() => handleUpdateItem(selectedItem.id, { syncSettings: { ...selectedItem.syncSettings, zenWaiting: !selectedItem.syncSettings.zenWaiting } })}
                          className={`w-12 h-6 rounded-full transition-colors relative ${selectedItem.syncSettings.zenWaiting ? "bg-indigo-500" : "bg-gray-700"}`}
                        >
                          <span className={`absolute top-1 left-1 w-4 h-4 rounded-full bg-white transition-transform ${selectedItem.syncSettings.zenWaiting ? "translate-x-6" : "translate-x-0"}`} />
                        </button>
                      </div>
                    )}
                  </div>
