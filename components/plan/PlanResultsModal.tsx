...
import { Plan, PlanStepResult } from "@/types/plan";
      ...
      >
        ...
        <div className="p-6 border-b border-gray-800 flex justify-between items-center bg-gray-900/50">
          <div>
            ...
            <div className="text-sm text-gray-400">User: <span className="text-sky-400 font-medium">{user.name}</span></div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-gray-700">
                  ...
                  <tr key={item.id} className="group hover:bg-white/5 transition-colors">
                    ...
                    <td className="py-4 pl-2">
                       <div className="flex items-center gap-3">
                         ...
                         <div>
                           <div className="font-medium text-gray-300 group-hover:text-white transition-colors">{item.metadata.title}</div>
                           <div className="text-xs text-gray-500">{item.metadata.subtitle}</div>
                         </div>
                       </div>
                    </td>
                    ...
                    <td className="py-4 text-right font-mono text-lg font-bold text-gray-200">
                      {isCompleted ? Math.round(result.wpm) : <span className="text-gray-700">-</span>}
                    </td>
                    <td className="py-4 text-right font-mono text-lg font-bold text-gray-200">
                       {isCompleted ? (
                         <span className={result.accuracy < 90 ? "text-red-400" : result.accuracy < 95 ? "text-yellow-400" : "text-green-400"}>
                           {Math.round(result.accuracy)}%
                         </span>
                       ) : <span className="text-gray-700">-</span>}
                    </td>
                    <td className="py-4 text-right pr-2 font-mono text-gray-500">
                       {isCompleted ? Math.round(result.raw || result.wpm) : <span className="text-gray-700">-</span>}
                    </td>
                  </tr>
                );
              })}
          ...
          {plan.length === 0 && (
             <div className="text-center text-gray-500 py-10">No plan steps found.</div>
          )}
        </div>
