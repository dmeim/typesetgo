import type { CategoryConfig, ThemeCategory, ThemeCatalogEntry, ThemeVariantSummary } from "../types/theme.ts";
...
export const CATEGORY_CONFIG: Record<ThemeCategory, CategoryConfig> = {
  ...
  "sports-teams": { displayName: "Sports Teams", order: 29 },
  "sports-teams-football": { displayName: "Sports Teams - Football", order: 30 },
  "sports-teams-basketball": { displayName: "Sports Teams - Basketball", order: 31 },
  "sports-teams-baseball": { displayName: "Sports Teams - Baseball", order: 32 },
  "sports-teams-soccer": { displayName: "Sports Teams - Soccer", order: 33 },
  "sports-teams-hockey": { displayName: "Sports Teams - Hockey", order: 34 },
  "sports-teams-college-football": { displayName: "Sports Teams - College Football", order: 35 },
  "sports-teams-college-basketball": { displayName: "Sports Teams - College Basketball", order: 36 },
  "sports-teams-college-baseball": { displayName: "Sports Teams - College Baseball", order: 37 },
  "sports-teams-college-soccer": { displayName: "Sports Teams - College Soccer", order: 38 },
  "sports-teams-college-hockey": { displayName: "Sports Teams - College Hockey", order: 39 },
  food: { displayName: "Food", order: 40 },
  fun: { displayName: "Fun", order: 41 },
  holiday: { displayName: "Holiday", order: 42 },
  // Collections/Misc
  zodiac: { displayName: "Zodiac", order: 43 },
  gemstones: { displayName: "Gemstones", order: 44 },
  instruments: { displayName: "Instruments", order: 45 },
  dance: { displayName: "Dance", order: 46 },
  vehicles: { displayName: "Vehicles", order: 47 },
  comics: { displayName: "Comics", order: 48 },
  "historical-era": { displayName: "Historical Eras", order: 49 },
  emotions: { displayName: "Emotions", order: 50 },
  textiles: { displayName: "Fabrics & Textures", order: 51 },
};
...
/** Shared by Vite and the isolated browser fixture; no palettes enter the index. */
export function createThemeCatalogEntry(id: string, source: unknown): ThemeCatalogEntry {
  const record = (value: unknown): Record<string, unknown> => {
    if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`Invalid theme metadata: ${id}`);
    return value as Record<string, unknown>;
  };
  const data = record(source);
  const definitions = data.variants ? Object.entries(record(data.variants)) : [["default", data]] as const;
  const variants: ThemeVariantSummary[] = definitions.map(([variantId, definition]) => {
    const variant = record(definition);
    const dark = record(variant.dark);
    const typing = record(dark.typing);
    const swatches = [record(dark.bg).base, typing.cursor, record(record(dark.interactive).secondary).DEFAULT, typing.correct];
    if (!swatches.every((color) => typeof color === "string")) throw new Error(`Invalid theme swatches: ${id}`);
    return {
      id: variantId,
      label: typeof variant.label === "string" ? variant.label : formatThemeName(variantId),
      light: variant.light != null,
      swatches: swatches as ThemeVariantSummary["swatches"],
    };
  });
  const defaultVariant = variants.find((variant) => variant.id === data.defaultVariant) ?? variants[0];
  if (!defaultVariant) throw new Error(`Theme has no variants: ${id}`);
  return {
    id,
