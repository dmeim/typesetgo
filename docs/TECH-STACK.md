# Tech Stack

Technology choices and versions powering TypeSetGo.

## Runtime

| Tool | Version | Role |
|------|---------|------|
| Bun | 1.3.3 (package manager pin) | Package manager and JS runtime |
| TypeScript | 7.0.2 | Type system across frontend and backend |
| Node target | ESM (`"type": "module"`) | Module format |

## Frontend

| Library | Version | Role |
|---------|---------|------|
| React | 19 | UI framework |
| Vite | 8 | Dev server, bundler, and build tool |
| React Router DOM | 7 | Client-side SPA routing |

Vite is configured with the `@vitejs/plugin-react` plugin and a custom `autoManifestPlugin` that generates `manifest.json` files for theme and asset directories.

The `@/` import alias maps to `src/`.

## Styling

| Library | Version | Role |
|---------|---------|------|
| Tailwind CSS | 4 | Utility-first CSS framework |
| PostCSS | 8 | CSS processing pipeline |
| Autoprefixer | 10 | Vendor prefix automation |
| tw-animate-css | 1.x | Tailwind animation utilities |

Tailwind v4 is integrated via `@tailwindcss/postcss` rather than the older plugin-based setup.

## UI Components

| Library | Version | Role |
|---------|---------|------|
| Shadcn/UI | 4.x (CLI) | Component system (new-york style, CSS variables, neutral base color) |
| Radix UI | various | Accessible headless primitives |
| Phosphor React | 2.1.10 | Unified UI icon library (`@phosphor-icons/react`); bold default, regular for lighter artwork |
| class-variance-authority | 0.7 | Component variant API |
| clsx | 2.x | Conditional class merging |
| tailwind-merge | 3.x | Tailwind-aware class deduplication |
| @base-ui/react | 1.8+ | Themed toast notifications |
| Embla Carousel | 8.x | Carousel component |
| Framer Motion | 13.x | Animation library |

Radix primitives in use: dialog, dropdown-menu, label, popover, progress, select, slider, slot, switch, tabs.

## Backend

| Library | Version | Role |
|---------|---------|------|
| Convex | 1.45+ | Real-time serverless backend (database, queries, mutations, actions) |

All backend logic lives in the `convex/` directory. The schema and indexes are defined in `convex/schema.ts`. Generated files in `convex/_generated/` should not be edited.

## Authentication

| Library | Version | Role |
|---------|---------|------|
| Clerk | 5.x (`@clerk/clerk-react`) | User authentication and session management |

Clerk is optional — it activates when `VITE_CLERK_PUBLISHABLE_KEY` is set.

## Forms & Validation

React state manages forms. Convex validators define backend contracts; typed preference adapters normalize stored settings. No form framework or Zod schema layer is used.

## Drag & Drop

| Library | Version | Role |
|---------|---------|------|
| @dnd-kit/core | 6.x | Drag-and-drop engine |
| @dnd-kit/sortable | 10.x | Sortable list preset |
| @dnd-kit/utilities | 3.x | Shared helpers |

## Charts

| Library | Version | Role |
|---------|---------|------|
| Recharts | 2.15 | Composable charting library built on D3 |

## Theming

The local `ThemeContext` owns theme, variant, and light/dark selection. JSON palettes live in `public/themes/`; Vite generates a compact browsing catalog and manifests. Individual palettes and fonts load on demand.

## Testing

| Library | Version | Role |
|---------|---------|------|
| convex-test | 0.0.59 | Isolated registered functions, schema/index validation and authenticated identities |
| Vitest | 4.x | Unit and integration test runner |
| @testing-library/react | 16.x | Component test utilities |
| @testing-library/jest-dom | 7.x | DOM assertion matchers |
| jsdom | 30.x | Browser environment for tests |
| Playwright | 1.62+ | End-to-end browser tests |

## Linting

| Library | Version | Role |
|---------|---------|------|
| ESLint | 10 (flat config) | Code linting |
| typescript-eslint | 8.x | TypeScript-aware lint rules |
| eslint-plugin-react-hooks | 7.x | Hooks rules enforcement |
| eslint-plugin-react-refresh | 0.5 | Fast refresh boundary validation |

## Hosting and deployment

Cloudflare Workers Static Assets serves `typesetgo.app` from Vite's generated `dist/` directory. Wrangler 4 manages deployment and the custom domain. Production Clerk and the existing Convex **development deployment** remain the live services; the hosting migration did not migrate database data.

Deployment currently runs locally with `bun run cf:deploy`. The owner previously reported connecting GitHub `main` through Workers Builds; a successful automated deployment is not recorded and remote settings were not rechecked. Treat pushes to main as potentially publishing. Docker/VPS deployment is retired. See the [deployment guide](deployment/CLOUDFLARE_GUIDE.md).
